...
module "iam_assumable_role_external_dns" {
  ...
  version                       = "~> v2.6.0"
  create_role                   = true
  role_name                     = "eks-external-dns-${data.terraform_remote_state.eks.outputs.cluster_id}-${random_id.random_role_id.hex}"
...
resource "aws_iam_policy" "external_dns" {
  name   = "AllowExternalDNSUpdates-${data.terraform_remote_state.eks.outputs.cluster_id}-${random_id.random_role_id.hex}"
  policy = <<-EOF
{
  "Version": "2012-10-17",
  "Statement": [
