...
module "iam_assumable_role_external_dns" {
  ...
  role_name                     = "eks-external-dns-${var.k8s_cluster_id}-${random_id.random_role_id.hex}"
...
resource "aws_iam_policy" "external_dns" {
  ...
  name   = "AllowExternalDNSUpdates-${var.k8s_cluster_id}-${random_id.random_role_id.hex}"
  policy = <<-EOF
{
  "Version": "2012-10-17",
  "Statement": [
